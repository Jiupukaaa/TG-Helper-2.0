...
import { createSavedGif, createSavedLink, createSavedPhoto, createSavedVoice, deleteSavedItem, detectSavedLink, getOwnedSavedItem, listSavedItems } from "@/services/savedService";
import { mainMenuKeyboard } from "@/bot/keyboards";

export const savedComposer = new Composer();
type SavedItem = { id: number; type: "LINK" | "PHOTO" | "GIF" | "VOICE"; url: string | null; fileId: string | null };
type Filter = "ALL" | "LINK" | "PHOTO" | "GIF" | "VOICE";
...
function itemLabel(item: Pick<SavedItem, "type" | "url">, number: number): string {
  ...
  if (item.type === "GIF") return `#${number} 🎞️ GIF`;
  return `#${number} 🎙️ Голосовое сообщение`;
}
...
function filterKeyboard(filter: Filter) {
  const k = new InlineKeyboard()
    ...
    .row()
    .text(filter === "VOICE" ? "✅ Голосовые" : "🎙️ Голосовые", "saved:filter:VOICE");
  return k;
}
...
async function renderSavedList(ctx: any, userId: number, edit = true, filter: Filter = "ALL") {
  ...
  if (all.length === 0) {
    const text = "🔖 Сохраненки\n\nПока здесь ничего нет.\n\nОтправьте ссылку YouTube, TikTok или Twitch, фото, GIF либо голосовое сообщение — я сохраню это сюда.";
  ...
  const counts = `Всего: ${all.length} · 🔗 ${all.filter((x) => x.type === "LINK").length} · 🖼️ ${all.filter((x) => x.type === "PHOTO").length} · 🎞️ ${all.filter((x) => x.type === "GIF").length} · 🎙️ ${all.filter((x) => x.type === "VOICE").length}`;
  const lines = items.map((item) => itemLabel(item, all.findIndex((x) => x.id === item.id) + 1));
  const keyboard = filterKeyboard(filter);

  // Action buttons are intentionally created only for media items.
  // Links remain visible in the list, but do not get an action button.
  items.forEach((item) => {
    ...
    if (item.type === "VOICE") keyboard.row().text(`🎙️ Отправить голосовое #${number}`, `saved:send:${item.id}`);
  });

  keyboard.row().text("🗑 Удалить", "saved:delete_mode").text("🗑 Удалить несколько", "saved:bulk").row().text("⬅️ В главное меню", "menu:main");
  const filterName = filter === "ALL" ? "Все" : filter === "LINK" ? "Ссылки" : filter === "PHOTO" ? "Фото" : filter === "GIF" ? "GIF" : "Голосовые";
...
savedComposer.callbackQuery(/^saved:filter:(ALL|LINK|PHOTO|GIF|VOICE)$/, async (ctx) => { await ctx.answerCallbackQuery(); const user = await getOrCreateUser(BigInt(ctx.from.id)); await renderSavedList(ctx, user.id, true, ctx.match[1] as Filter); });
...
savedComposer.callbackQuery(/^saved:send:(\d+)$/, async (ctx) => {
  ...
  if (!item?.fileId) { await ctx.reply("Сохраненный файл не найден."); return; }
  ...
  else if (item.type === "VOICE") await ctx.api.sendVoice(ctx.chat!.id, item.fileId);
  else await ctx.reply("Для этого элемента нет кнопки действия.");
});
...
export async function handleSavedVoiceInput(ctx: any, userId: number): Promise<boolean> { const voice = ctx.message?.voice; if (!voice?.file_id) return false; const saved = await createSavedVoice(userId, voice.file_id); const items = await listSavedItems(userId); const number = items.findIndex((item) => item.id === saved.id) + 1; await ctx.reply(`🔖 Сохраненка #${number} — 🎙️ Голосовое сообщение сохранено.`, { reply_markup: mainMenuKeyboard }); return true; }
