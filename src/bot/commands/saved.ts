...
async function renderSavedList(ctx: any, userId: number, edit = true, filter: Filter = "ALL") {
  ...
  const lines = items.map((item, index) => itemLabel(item, all.findIndex((x) => x.id === item.id) + 1));
...
savedComposer.callbackQuery(/^saved:bulk_toggle:(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const user = await getOrCreateUser(BigInt(ctx.from.id)); const id = Number(ctx.match[1]); const sessionKey = `savedBulk:${user.id}`; const { getSession, setSessionStep } = await import("@/bot/session"); const session = await getSession(user.id); const draft = (session?.draft as { ids?: number[] } | null) ?? {}; const ids = new Set(draft.ids ?? []); ids.has(id) ? ids.delete(id) : ids.add(id); await setSessionStep(user.id, "IDLE" as any, { ids: [...ids] }); const items = await listSavedItems(user.id); const k = new InlineKeyboard(); items.forEach((item, index) => { k.text(`${ids.has(item.id) ? "☑" : "☐"} #${index + 1}`, `saved:bulk_toggle:${item.id}`); if (index % 2 === 1) k.row(); }); if (items.length % 2 === 1) k.row(); k.text(`🗑 Удалить выбранные (${ids.size})`, "saved:bulk_confirm").row().text("❌ Отмена", "saved:list"); await ctx.editMessageText("🗑 Выберите сохраненки для удаления:", { reply_markup: k }); });
